...
pub async fn run(
    ...
    println!("Deposit to Kamino Lend");
    ...
    let serialized_tx =
        api::build_transaction(client, "deposit", &user_pubkey, market_pubkey, reserve, amount)
            .await?;
    ...
    if dry_run {
        println!("[dry-run] Deposit transaction built successfully (not submitted).");
        println!("  Serialized TX (base64): {}...", &serialized_tx[..serialized_tx.len().min(40)]);
    } else {
        println!("Deposit submitted!");
