...
pub async fn run(
    ...
    let serialized_tx =
        api::build_transaction(client, "repay", &user_pubkey, market_pubkey, reserve, amount)
            .await?;
    ...
    if dry_run {
        println!("[dry-run] Repay transaction built successfully (not submitted).");
        println!("  Serialized TX (base64): {}...", &serialized_tx[..serialized_tx.len().min(40)]);
    } else {
        println!("Repay submitted!");
