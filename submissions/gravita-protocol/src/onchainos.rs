...
pub fn resolve_wallet(chain_id: u64) -> anyhow::Result<String> {
    ...
    // Try tokenAssets[0].address first (populated when wallet has tokens)
    ...
    // Try data.address directly
    if let Some(addr) = json["data"]["address"].as_str() {
        if !addr.is_empty() {
            return Ok(addr.to_string());
        }
    }
    // Fallback: use `onchainos wallet addresses` to find EVM address for chain
    ...
    if let Some(evm_list) = addr_json["data"]["evm"].as_array() {
        ...
        // If no exact chain match, return the first EVM address (EVM addresses are shared)
        if let Some(entry) = evm_list.first() {
            if let Some(addr) = entry["address"].as_str() {
                if !addr.is_empty() {
                    return Ok(addr.to_string());
                }
