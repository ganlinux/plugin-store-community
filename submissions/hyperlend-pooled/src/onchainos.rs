...
/// Resolve the current logged-in EVM wallet address for chain_id.
/// Uses 3 fallbacks:
///   1. data.details[0].tokenAssets[0].address
///   2. data.address
///   3. onchainos wallet addresses -> data.evm[] entries
pub fn resolve_wallet(chain_id: u64) -> anyhow::Result<String> {
    ...
    if let Some(evm_addrs) = addr_json["data"]["evm"].as_array() {
        ...
        // All EVM chains share the same address — take first entry
        for entry in evm_addrs {
            if let Some(a) = entry["address"].as_str() {
                if !a.is_empty() {
                    return Ok(a.to_string());
...
/// ERC-20 approve(spender, amount) encoded as calldata.
pub async fn erc20_approve(
    ...
    let spender_padded = format!("{:0>64}", spender_hex);
