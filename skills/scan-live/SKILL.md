...
name: scan-live
description: >
  Chain Scanner Strategy v1.0 — Agentic Wallet TEE signing automated trading bot.
  ...
  Triggers when the user mentions chain scanner v2, chain scanner strategy,
  memepump scan, Tranches scan, pump.fun chain scan, safety filter chain scan,
  dev rug detection, bundler filter, on-chain scanning strategy, scan v2/v6,
...
1. **Extreme Risk Warning**: Chain Scanner Strategy targets newly launched small-cap Meme tokens, which represent **the highest-risk trading type** in cryptocurrency. Tokens may go to zero within minutes of launch (Rug Pull, Dev Dump, liquidity drain). You may lose your entire invested capital.
...
```
🔍 Chain Scanner Strategy v1.0 — Solana Meme Automated Trading Bot
